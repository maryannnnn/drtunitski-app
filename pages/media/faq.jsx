...
const MediaFaqPage = ({ initialData, seoData }) => {
    ...
    console.log('MediaFAQ - Total medias loaded:', allMedias.length);
    console.log('MediaFAQ - Current locale:', locale);
    
    let filteredMedias = filterByLanguage(allMedias, locale);
    
    console.log('MediaFAQ - Filtered for locale', locale, ':', filteredMedias.length);
    
    if (filteredMedias.length === 0) {
        console.warn('⚠️ MediaFAQ - No posts found for locale', locale, ', showing all posts as fallback');
        filteredMedias = allMedias;
    }
    
    // 🎯 ФИЛЬТРУЕМ ТОЛЬКО ВОПРОСЫ (FAQ, Questions, Вопросы)
    filteredMedias = filteredMedias.filter(item => {
        const trees = item.node.trees?.edges || [];
        return trees.some(tree => {
            const name = tree.node.name ? tree.node.name.toLowerCase() : '';
            return name.includes('question') || name.includes('faq') || name.includes('вопрос');
        });
    });
    
    console.log('MediaFAQ - After FAQ filter:', filteredMedias.length);
    ...
    const PageProps = {
        title: seoData?.title || t('mediaFaq.seoTitle') || 'Frequently Asked Questions',
        description: seoData?.description || t('mediaFaq.seoDescription') || 'Answers to common medical questions'
    };

    const breadcrumbsMaterial = {
        title: t('mediaFaq.title') || 'FAQ',
        slug: 'faq'
    };

    return (
        <MainLayout title={PageProps.title} description={PageProps.description} schemaType="FAQPage">
            <div className="media-blog" dir={dir}>
                <div className="container">
                    <h1 className="media-blog__title">{t('mediaFaq.title') || 'Frequently Asked Questions'}</h1>
                    ...
                    {error && (
                        <div className="media-blog__error">
                            <p>Error loading FAQ: {error.message}</p>
                        </div>
                    )}

                    {currentMedias.length === 0 ? (
                        <div className="media-blog__empty">
                            <p>{t('mediaFaq.noPosts') || 'No questions available'}</p>
                        </div>
                    ) : (
                        <>
...
export async function getStaticProps({ locale }) {
    ...
    const seoData = getSeoData('mediaFaq', locale);
