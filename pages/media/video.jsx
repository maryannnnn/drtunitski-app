...
const MediaVideoPage = ({ initialData }) => {
    ...
    console.log('MediaVideo - Total medias loaded:', allMedias.length);
    console.log('MediaVideo - Current locale:', locale);
    
    let filteredMedias = filterByLanguage(allMedias, locale);
    
    console.log('MediaVideo - Filtered for locale', locale, ':', filteredMedias.length);
    
    if (filteredMedias.length === 0) {
        console.warn('⚠️ MediaVideo - No posts found for locale', locale, ', showing all posts as fallback');
    ...
    filteredMedias = filteredMedias.filter(item => {
        ...
        return trees.some(tree => 
            tree.node.name && tree.node.name.toLowerCase().includes('video')
        );
    });
    
    console.log('MediaVideo - After video filter:', filteredMedias.length);
    ...
    const breadcrumbsMaterial = {
        title: t('mediaVideo.title') || 'Videos',
        slug: 'video'
    };
        ...
        <LeftLayout title={PageProps.title} description={PageProps.description}>
            <div className="media-blog" dir={dir}>
                <div className="container">
                    <h1 className="media-blog__title">{t('mediaVideo.title') || 'Video Gallery'}</h1>
                    ...
                    {currentMedias.length === 0 ? (
                        <div className="media-blog__empty">
                            <p>{t('mediaVideo.noPosts') || 'No videos available'}</p>
                        </div>
                    ) : (
                        <>
